...
async function UpdateMessage() {
    ...
    for (i = 0; i < receivedMessages.receivedMessageItems.length; i++) {
        ...
        if (i == nbError) {
            console.log("error impossible to update :" + message.messageText)
        }
        else {
            console.log(" message: ", message.messageText);
            await queueClient.updateMessage(message.messageId, message.popReceipt, "This message has been updated " + i);
        }
    }
}